...
class BaseModel(pl.LightningModule):
    ...
    def bce_loss_contractive_fn(self, predict, label, pos_weight, anchor, pos, neg):
        predict = predict.reshape(-1)
        label = label.reshape(-1)
        weight = pos_weight * label + 1 - label
        bce_loss = F.binary_cross_entropy(input=predict, target=label, weight=weight)
        tloss =  torch.nn.functional.relu(torch.sum((anchor-pos) ** 2, dim=-1) - torch.sum((anchor-neg) ** 2, dim=-1) +0.4).sum()
        contractive_loss = (pos - neg).mean()
        loss = 0.7*bce_loss + 0.3*tloss
        loss = bce_loss
        loss = 0.5*bce_loss + 0.5*contractive_loss
        return {"loss_bce":bce_loss,
                # "loss_con":contractive_loss,
                "loss":loss}
