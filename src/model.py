...
from src.model_help import BaseModel
from src import MODEL_REGISTRY, gnn
from GMD.model import Encoder, Generator, Discriminator, GaussianGenerator
import numpy as np
...
class HNNCL(BaseModel):
    ...
    def __init__(self, size_u, size_v,
        ...
        self.loss_fn = partial(self.bce_loss_contractive_fn, pos_weight=self.pos_weight)
        self.dis = Discriminator(256, 100, 0).cuda()
    ...
    def step(self, batch:FullGraphData):
        ...
        print(label.shape)
        #print(x)
        np.save("label.npy",label.cpu().detach().numpy())
        # predict, u, v, enc_prob, gen_prob = self.forward(x, u_edge_index, u_edge_weight, v_edge_index, v_edge_weight,
        #         ur_edge_index, ur_edge_weight, vr_edge_index, vr_edge_weight)
        predict, u, v, a,p,n = self.forward(x, u_edge_index, u_edge_weight, v_edge_index, v_edge_weight,
                ur_edge_index, ur_edge_weight, vr_edge_index, vr_edge_weight)
        ...
        ans = self.loss_fn(predict=predict, label=label, anchor = a, pos=p, neg = n)
    ...
    def forward(self, x, u_edge_index, u_edge_weight, v_edge_index, v_edge_weight,
        ...
        for inter_encoder, intra_encoder in zip(self.inter_encoders, self.intra_encoders):
            ...
            # inter_feature = inter_feature + intra_feature
            x = intra_feature + inter_feature + layer_out[-1]
            # inter_feature = inter_feature + intra_feature
            pos.append(intra_feature + inter_feature)
            # anchor = short_embedding
            # for i in inter_feature.shape[0]:

            neg.append(inter_feature + torch.rand(inter_feature.shape).cuda())

            layer_out.append(x)


        x = torch.stack(layer_out[1:])
        a = torch.stack(a).view(-1,128)
        p = torch.stack(pos).view(-1,128)
        n = torch.stack(neg).view(-1,128)
        # enc_prob = self.dis(torch.cat((a, p), dim=-1))#正样本对
        # gen_prob = self.dis(torch.cat((a, n), dim=-1))#负样本对
        ...
        # print(x.shape)
        #print(x)
        np.save("embedding.npy",x.cpu().detach().numpy())
        score, u, v = self.decoder(x)
        # score = torch.sigmoid(score)
        return score, u, v, a, p, n
        # return score, u, v,enc_prob, gen_prob
